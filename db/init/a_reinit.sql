...
-- 删除业务用户
DROP
USER IF EXISTS qiliblog_readwrite;
DROP
USER IF EXISTS qiliblog_readonly;
...
ALTER
DEFAULT PRIVILEGES FOR ROLE qiliblog_owner REVOKE ALL ON TABLES FROM qiliblog_role_readwrite;
ALTER
DEFAULT PRIVILEGES FOR ROLE qiliblog_owner REVOKE ALL ON SEQUENCES FROM qiliblog_role_readwrite;
ALTER
DEFAULT PRIVILEGES FOR ROLE qiliblog_owner REVOKE
SELECT
ON TABLES
FROM qiliblog_role_readonly;
REVOKE ALL ON SCHEMA qiliblog FROM qiliblog_role_readwrite;
-- 谨慎操作！此命令会删除该角色在当前数据库下拥有的所有对象
DROP
OWNED BY qiliblog_role_readwrite;
REVOKE ALL ON SCHEMA qiliblog FROM qiliblog_role_readonly;
-- 谨慎操作！此命令会删除该角色在当前数据库下拥有的所有对象
DROP
OWNED BY qiliblog_role_readonly;

-- 删除角色（在 qiliblog 数据库连接下尝试，或切换回 postgres 后执行）
\c
postgres; -- 删除角色通常可在任意数据库连接下执行，切换回 postgres 是良好实践
